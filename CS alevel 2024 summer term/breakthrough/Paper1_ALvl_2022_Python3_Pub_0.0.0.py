...
class Breakthrough():
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    if MenuChoice == "D":
                        ...
                        print(f"{self.__Deck.GetCardDescriptionAt(0)}, {self.__Deck.GetCardDescriptionAt(1)}, {self.__Deck.GetCardDescriptionAt(2)}")
                        self.__CurrentLock.SetPeekUsed()
                    elif MenuChoice == "M":
                        if not self.__MulliganUsed:
                            for count in range (self.__Sequence.GetNumberOfCards()):
                                self.__MoveCard(self.__Sequence, self.__Deck, self.__Sequence.GetCardNumberAt(0))
                            ...
                            for count in range (self.__Hand.GetNumberOfCards()):
                                self.__MoveCard(self.__Hand, self.__Deck, self.__Hand.GetCardNumberAt(0))
                            self.__Deck.Shuffle()
                            for count in range (5):
                                while self.__Deck.GetCardDescriptionAt(0) == 'Dif':
                                    self.__MoveCard(self.__)
    ...
    def __PlayCardToSequence(self, CardChoice):
        if self.__Sequence.GetNumberOfCards() > 0:
            if self.__Hand.GetCardDescriptionAt(CardChoice - 1)[0] != self.__Sequence.GetCardDescriptionAt(self.__Sequence.GetNumberOfCards() - 1)[0]:
                ...
                print("You can't play the same type of card. Please play another card.")
        else:
            self.__Score += self.__MoveCard(self.__Hand, self.__Sequence, self.__Hand.GetCardNumberAt(CardChoice - 1))
            self.__GetCardFromDeck(CardChoice)
    ...
    def __GetChoice(self):
        print()
        if not self.__CurrentLock.GetPeekUsed() and not self.__MulliganUsed:
            Choice = input("(D)iscard inspect, (U)se card, (P)eek top three cards, (M)ulligan:> ").upper()
        elif not self.__CurrentLock.GetPeekUsed() and self.__MulliganUsed:
            Choice = input("(D)iscard inspect, (U)se card, (P)eek top three cards:> ").upper()
        elif self.__CurrentLock.GetPeekUsed() and not self.__MulliganUsed:
