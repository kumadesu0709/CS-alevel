...
class Breakthrough():
    ...
    def PlayGame(self):
        if len(self.__Locks) > 0:
            self.__SetupGame()
            gameover = False
            while not self.__GameOver and not gameover:
                self.__LockSolved = False
    ...
    def __GetChoice(self):
        ...
        if not self.__CurrentLock.GetPeekUsed() and not self.__MulliganUsed:
            Choice = input("(D)iscard inspect, (U)se card, (P)eek top three cards, (M)ulligan, (Q)uit:> ").upper()
        elif not self.__CurrentLock.GetPeekUsed() and self.__MulliganUsed:
            Choice = input("(D)iscard inspect, (U)se card, (P)eek top three cards,(Q)uit:> ").upper()
        elif self.__CurrentLock.GetPeekUsed() and not self.__MulliganUsed:
            Choice = input("(D)iscard inspect, (U)se card, (M)ulligan, (Q)uit:> ").upper()
        else: 
