...
class CardCollection():
    ...
    def Update(self):
        ...
        for cards in self._Cards:
            if cards.GetDescription()[0] == "F":
                self._NumFiles += 1
            elif cards.GetDescription()[0] == "K":
                self._NumKeys += 1
            elif cards.GetDescription()[0] == "P":
                self._NumPicks += 1
    
    def DisplayStats(self):
        fPos = self._NumFiles / self.GetNumberOfCards()
        kPos = self._NumKeys / self.GetNumberOfCards()
        pPos = self._NumPicks / self.GetNumberOfCards()
        return f'There is a {kPos:.2f}% chance that the next card will be a key, a {fPos:.2f}% chance that it will be a file and a {pPos:.2f}% chance
that it will be a pick.'
