...
class Company:
  ...
  def ProcessDayEnd(self):
    ...
    Details += "Daily costs for company: " + str(round(self._DailyCosts,2)) + "\nCost for delivering produce to outlets: " + str(round(DeliveryCosts,2)) + "\n"
    for Current in range (0, len(self._Outlets)):
      ...
      Details += "Outlet " + str(Current + 1) + " profit/loss: " + str(round(ProfitLossFromThisOutlet,2)) + "\n"
      ProfitLossFromOutlets += ProfitLossFromThisOutlet
    Details += "Previous balance for company: " + str(round(self._Balance,2)) + "\n"
    self._Balance += ProfitLossFromOutlets - self._DailyCosts - DeliveryCosts
    Details += "New balance for company: " + str(round(self._Balance,2))
    if self._Balance < 0:
        Details += "\n" + "You're bankrupt. The company is now closed."
  ...
  def Run(self):
    ...
    while Choice != "Q":
      ...
      if Choice == "1":
        ...
        days = int(input("how many days do you want to jump?")) + 1
