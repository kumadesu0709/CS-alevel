...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    Score = 0
    GameOver = False
    freeze_position = 9999999
    have_freeze = False
    while not GameOver:
        DisplayState(Targets, NumbersAllowed, Score, freeze_position)
        UserInput = input("Enter an expression, or enter freeze or unfreeze: ")
        print()
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                ...
                if IsTarget:
                    ...
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
        elif UserInput == "freeze":
            if have_freeze == False:
                freeze_position = int(input("What is the position of the number that you want to freeze?"))-1
                have_freeze = True
        elif UserInput == "unfreeze":
            if have_freeze == True:
                freeze_position = 99999999
                have_freeze = False
        Score -= 1
        if Targets[0] != -1:
            GameOver = True
        else:
            Targets = UpdateTargets(Targets, TrainingGame, MaxTarget, freeze_position)        
    print("Game over!")
    DisplayScore(Score)
...
def UpdateTargets(Targets, TrainingGame, MaxTarget, freeze_position):
    frozen_number = Targets[freeze_position]
    Targets.pop(freeze_position)
    for Count in range (0, len(Targets) - 1):
        Targets[Count] = Targets[Count + 1]
    Targets.insert(freeze_position, frozen_number)
    Targets.pop()
    if TrainingGame:
...
def DisplayState(Targets, NumbersAllowed, Score, freeze_position):
    DisplayTargets(Targets, freeze_position)
...
def DisplayTargets(Targets, freeze_position):
    print("|", end = '')
    for i in range(len(Targets)):
        if Targets[i] == -1:
            print(" ", end = '')
        elif i == freeze_position:
