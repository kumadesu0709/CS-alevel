...
def ConvertToRPN(UserInput):
    ...
    while Position < len(UserInput):
        if UserInput[Position] == "(":
            # Found an opening bracket so scan forward to find its closing partner
            Position+=1
            bracket_input = []
            opening_bracket_count = 0
            while UserInput[Position] != ")" or opening_bracket_count > 0:
                ## Need to count opening brackets in order to find the correct close bracket
                if UserInput[Position] == "(":
                    opening_bracket_count += 1
                elif UserInput[Position] == ")":
                    opening_bracket_count -= 1
                bracket_input.append(UserInput[Position])
                Position += 1
            # Convert the contents of the bracket into RPN and add to the end of the final RPN string
            bracket_in_RPN = ConvertToRPN(''.join(bracket_input))
            UserInputInRPN.extend(bracket_in_RPN)
            Position+=2
        else:
            Operand, Position = GetNumberFromUserInput(UserInput, Position)
            UserInputInRPN.append(str(Operand))
